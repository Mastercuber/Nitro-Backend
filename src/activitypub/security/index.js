...
import { activityPub } from '../../server'
    ...
    if (result.error) {
      ...
      request({
        ...
        headers: {
          'Host': targetDomain,
          'Date': date,
          'Signature': createSignature(privateKey, fromName, url, { 'Host': targetDomain, 'Date': date, 'Content-Type': 'application/activity+json' }),
...
function createSignature (privKey, name, url, headers = {}, algorithm = 'rsa-sha256') {
  ...
  return `keyId="http://${activityPub.domain}/activitypub/users/${name}#main-key",algorithm="${algorithm}",headers="(request-target)${headersString}",signature="${signatureB64}"`
}
